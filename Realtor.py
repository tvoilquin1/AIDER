...
def main():
    area, max_price, min_bedrooms, min_bathrooms = get_search_criteria()
    url, headers, querystring = build_request(area, max_price, min_bedrooms, min_bathrooms)
...
if __name__ == "__main__":
    main()
def validate_input(area, max_price, min_bedrooms, min_bathrooms):
    if not area or not max_price or not min_bedrooms or not min_bathrooms:
        print("All fields are required.")
        return False
    if not max_price.isdigit() or not min_bedrooms.isdigit() or not min_bathrooms.isdigit():
